...
impl<R: Reproducibility> TripleMixSimdCore<R> {
    ...
    pub(crate) fn fill_blocks(&mut self, blocks: &mut [[u64; BLOCK_SIZE]]) {
        ...
        for block in blocks {
            ...
            let pcg_p2_shifted_hi =
                (pcg_p2_hi << Simd64::splat(32)) | (pcg_p2_lo >> Simd64::splat(32));

            // low sum = p1_lo + p2_shifted_lo
            let (pcg_prod_lo, pcg_carry) =
                Self::add128_with_carry(pcg_p1_lo, pcg_p2_shifted_lo, Simd64::splat(0));
    ...
    fn first_half_mix(scalar1: u64, scalar2: u64, x0: Simd32, x1: Simd32, x2: Simd32, x3: Simd32, x4: Simd32) -> (Simd32, Simd32, Simd32) {
        ...
        let scalar_mix_1 =
            Simd32::from_array([
                scalar2_lo * scalar1_hi, scalar1_lo, scalar1_hi, 0, scalar1_hi.wrapping_sub(scalar2_hi), scalar2_lo, scalar1_lo ^ scalar1_hi, scalar2_hi,
            ]);
        let mut b = Simd32::splat(0x9e3779b9);
        let scalar_mix_2 =
            Simd32::from_array([
                scalar1_hi, scalar2_hi.wrapping_add(scalar1_lo), scalar2_hi, scalar1_hi ^ scalar2_lo, scalar1_lo * scalar2_hi, scalar1_lo, 0, scalar2_lo,
            ]);
        a ^= scalar_mix_1;
...
mod tests {
    ...
    fn mix_from_flat_array(mix_input: [u64; MIX_INPUT_U64S]) -> (Simd64, Simd64, Simd64, Simd64) {
        ...
        let mut output = [0u64; BLOCK_SIZE];
        ...
        (Simd::from_array(output[0..4].try_into().unwrap()),
        Simd::from_array(output[4..8].try_into().unwrap()),
        Simd::from_array(output[8..12].try_into().unwrap()),
        Simd::from_array(output[12..16].try_into().unwrap()))
    }
    ...
    fn test_mix_matrix_random_inputs() {
        ...
        let grand_sigma =
            ((AVALANCHE_MATRIX_ROWS * AVALANCHE_MATRIX_COLS * RANDOM_INPUT_ITERATIONS) as f64
                * 0.25
                - 1.0)
                .sqrt();
        for _ in 0..RANDOM_INPUT_ITERATIONS {
            ...
            assert!(
                z >= -4.0,
                "Total weight {total_weight} (z={z}) too low (rank {rank} input)"
            );
            assert!(
                z <= 4.0,
                "Total weight {total_weight} (z={z}) too high (rank {rank} input)"
            );
        }
    ...
    fn test_4_step_matrix_rank_distribution_miri_xslow() {
        ...
        for _ in 0..iterations {
            ...
            for mut_field in fields {
                for lane in 0..SIMD_WIDTH {
                    for bit in 0..64 {
                        ...
                        // Record differences
                        for (step_idx, (base_block, flipped_block)) in
                            base_outputs.iter().zip(flipped_outputs.iter()).enumerate()
                        {
                            for (word_idx, (&base_word, &flipped_word)) in
                                base_block.iter().zip(flipped_block.iter()).enumerate()
                            {
            ...
            if iterations > 1 {
                ...
                assert!(
                    rank >= STATE_BITS - 5,
                    "Rank too low for {STATE_BITS} state bits: {}",
                    rank
                );
            }
        }
        if iterations > 1 {
            ...
            assert!(
                mean_rank >= (STATE_BITS as f64 - 4.5),
                "Mean rank too low for {STATE_BITS} state bits: {:.2}",
                mean_rank
            );
        }
    }
