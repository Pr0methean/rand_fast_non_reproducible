...
#[library_benchmark]
#[bench::triple_mix_prng_not_reproducible(TripleMixPrng::<NotReproducible>::from_rng(&mut UnwrapErr(SysRng)), &mut [0u64; BLOCK_SIZE])]
#[cfg_attr(feature = "reproducibility_same_endianness", bench::triple_mix_prng_same_endianness(TripleMixPrng::<SameEndianness>::from_rng(&mut UnwrapErr(SysRng)), &mut [0u64; BLOCK_SIZE]))]
#[cfg_attr(feature = "reproducibility_cross_platform", bench::triple_mix_prng_cross_platform(TripleMixPrng::<CrossPlatform>::from_rng(&mut UnwrapErr(SysRng)), &mut [0u64; BLOCK_SIZE]))]
#[cfg_attr(feature = "bench_include_threadrng", bench::thread_prng(rand::rng(), &mut [0u64; BLOCK_SIZE]))]
fn fill_16k<T: Rng>(mut prng: T, buffer: &mut [u64; BLOCK_SIZE]) -> &[u64; BLOCK_SIZE] {
